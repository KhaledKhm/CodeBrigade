 ...
 * @author IBM
 */
public class ListEntretiensForm extends Demo {
  private long lastScroll;
    private ArrayList<Entretien> list;
    
   
    private Resources res;
    private boolean messageShown;
 
     public ListEntretiensForm() {
    ...
    public Container createDemo(Form parentForm) {
        ...
        list.addAll(ServiceEntretien.getInstance().getAllEntretiens());
        for (Entretien t : list) {

            MultiButton mb = new MultiButton(t.getLibelle());
            mb.setTextLine2(t.getDateentretien());
            String f = String.valueOf(t.getIdevaluation()) ; 
            String g = String.valueOf(t.getIdParticipant()) ; 
            mb.setTextLine3(f);
            mb.setTextLine3(g);
          ...
          //     new ComplaintForm(parentForm, t.getId()).show()  ; 
                  ...
                  MultiButton mbc = new MultiButton(t.getDateentretien()) ; 
                  mbc.setTextLine2(t.getDescription()) ; 
                  mbc.setTextLine2(t.getLibelle()) ; 
                  String x= String.valueOf(t.getIdevaluation()) ; 
                  mbc.setTextLine2(x) ; 
                  mbc.setTextLine2(g) ; 
                ...
                TextField Dateentretien = new TextField("", "New Date Entretien");
                TextField Description = new TextField("", "New Description");
                TextField idParticipant = new TextField("", "New Participant Id");
                TextField Idevaluation = new TextField("", "New Evaluation Id");
                Dialog abcd = new Dialog("Update Entretien");
                BoxLayout bo = new BoxLayout(CENTER);
                abcd.setLayout(bo);
                Button oki = new Button("Update");
                abcd.add(Libelle).add(Description).add(Dateentretien).add(idParticipant).add(Idevaluation).add(oki);//add text field to dialog

                abcd.setDisposeWhenPointerOutOfBounds(true);
                abcd.setBackCommand(new Command(""));

                oki.addActionListener(evtt -> {
                    if ((Libelle.getText().length() == 0)||(Dateentretien.getText().length() == 0)) {
                        Dialog.show("Alert", "Empty Field ", new Command("OK"));
                    } else {
                        
                        Entretien j2 = new Entretien(Integer.parseInt(Idevaluation.getText()),Integer.parseInt(idParticipant.getText()),Libelle.getText(), Dateentretien.getText(), Description.getText()); 
                        String s = String.valueOf(t.getId()) ; 
                        if (ServiceEntretien.getInstance().updateEntretien( j2 , s)) {
                            abcd.dispose();
                            ToastBar.showMessage("Updating Entretien...", FontImage.MATERIAL_INFO, 3000);
...
//            });
            ...
            delete.addActionListener(e -> {
                if (Dialog.show("Delete", "Are you sure?\n", "Delete", "Cancel")) {
                    String s= String.valueOf(t.getId()) ; 
                    if (ServiceEntretien.getInstance().deleteEntretien(s)) {
